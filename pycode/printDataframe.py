...
st.sidebar.info('Welcome!! This app will allow users to search for a content within videos using the labels generated by the Google VideoIntelligence API. Two types of content detection has been used: Object/label and Text. Choose a option to see the list of labels under them. From the grid please select a row to view the relevant video from the correct segment')
